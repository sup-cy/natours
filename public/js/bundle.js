(() => {
  ...
  var displayMap = (locations) => {
    ...
    locations.forEach((loc) => {
      ...
      L.marker([loc.coordinates[1], loc.coordinates[0]]).addTo(map2).bindPopup(`<p>Day ${loc.day}: ${loc.description}</p>`, {
        autoClose: false,
        className: "mapPopup"
      }).openPopup();
    });
  ...
  var updatePassword = async (passwordCurrent, password, passwordConfirm) => {
    try {
      const res = await axios_default({
        method: "PATCH",
